@if(isset($href))
    <a wire:navigate {{ $attributes->merge(['class' => "btn group/button"]) }}>
        ...
        <x-heroicon-o-arrow-small-right class="size-6 absolute top-1/2 left-1/2 group-hover/button:-translate-x-1/2 -translate-x-full -translate-y-1/2 opacity-0 group-hover/button:opacity-100 transition-all" />
    </a>
@else
