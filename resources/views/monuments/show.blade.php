...
@section('content')
    ...
    <div
        ...
        <div class="flex flex-col sm:flex-row items-start gap-8 md:gap-16">
            <div
                class="flex items-center justify-end w-full sm:sticky sm:z-10 sm:top-[calc(var(--nav-height)+var(--subheader-height))] sm:w-auto sm:gap-3 sm:flex-col">
                <p class="text-xs grow">
                    {{ __('common.share') }}
                </p>
                <button type="button"
                        @click="navigator.share({ title:`{{ __('monument.share') }}`, url:'{{ LaravelLocalization::getNonLocalizedURL(URL::current()) }}'})"
                ...
                <a href="https://twitter.com/intent/tweet?text={{ __('monument.share') }}&url={{ LaravelLocalization::getNonLocalizedURL(URL::current()) }}"
                   target="_blank" class="p-3 hover:bg-gray-100 rounded-full sm:border">
