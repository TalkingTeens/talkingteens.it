...
<div>
    <div class="sticky top-4 bg-white z-40 shadow">
        <div class="mx-auto max-w-7xl w-11/12 pb-3 pt-4 flex justify-between items-center gap-x-4">
            <div class="overflow-x-auto max-w-6xl flex gap-x-10">
                <x-button.category title="Tutto" icon="svg/grid.svg" alt="Icona" />
                @foreach($categories as $category)
                    <x-button.category :title="$category->name"
                                       :category="$category->slug"
                                       :icon="Storage::url($category->icon)"
                                        alt="Icona"/>
                @endforeach
            </div>
            <div class="flex justify-center items-center gap-x-2 cursor-pointer" wire:click="toggleView()">
                <p class="text-sm font-medium">
                    {{ $view == 'list' ? 'Visita la mappa' : 'Vedi la lista' }}
                </p>
                @if($view == 'list')
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="h-6 w-6">
                        <path d="M464 256c0-114.87-93.13-208-208-208S48 141.13 48 256s93.13 208 208 208 208-93.13 208-208z" fill="none" stroke="currentColor" stroke-miterlimit="10" stroke-width="32"/>
